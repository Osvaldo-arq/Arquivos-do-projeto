...
def process_text(text_data):
    ...
    patterns = {
        ...
        "valor_total": r"TOTAL\s*R\$\s*:?\s*([\d\.,]+)",
        "nome_emissor": r"^(.*?)(?=\s*CNPJ|\s*End\.|\s*Rua|\s*Avenida|\s*Av\.|\s*CEP|\s*LTDA)",
        "endereco_emissor": r"(?:End\.:\s*|Rua\s*|Avenida\s*|Av\.|AVENIDA\s*|Rodovia\s*|BR|RODOVIA)[\s\S]*?(?=\s*CEP \d{5}-\d{3}|\s*CNPJ|\s*TEL)",
